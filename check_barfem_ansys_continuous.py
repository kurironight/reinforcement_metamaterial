...
from check_fem_by_ansys.preprocess import make_random_fem_condition_with_continuous_graph
...
eps = 1.0e-8  # 共益勾配法の許容閾値

for i in tqdm(range(900)):
    nodes_pos, edges_indices, edges_thickness,\
        input_nodes, input_vectors, frozen_nodes = \
        make_random_fem_condition_with_continuous_graph(max_node_num, max_edge_pos)
    output_nodes = [5]
    output_vectors = np.array([[1, 1]])
    log_dir = "check_fem_by_ansys/continuous_conditions/"
    ...
    if not result:
        error_num += 1
        log_dir = "check_fem_by_ansys/continuous_conditions/error{}".format(error_num)
        os.makedirs(log_dir, exist_ok=True)
        np.save(os.path.join(log_dir, 'nodes_pos.npy'), nodes_pos)
        np.save(os.path.join(log_dir, 'edges_indices.npy'), edges_indices)
        np.save(os.path.join(log_dir, 'edges_thickness.npy'), edges_thickness)
