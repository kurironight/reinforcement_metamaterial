...
last_data = 100  # 各段階的GAで保存されている最後のepoch
...
for i in tqdm(range(data_num)):
    ...
    for j in range(len(save_epochs) - 1):
        dir = os.path.join(load_dir, "{}/free_{}_fix_8/{}".format(i, max_node_num, save_epochs[j]))
        if not os.path.exists(os.path.join(dir, 'nodes_pos.npy')):
            continue

        if flag:
            G1 = make_G(dir)
            dir = os.path.join(load_dir, "{}/free_{}_fix_8/{}".format(i, max_node_num, save_epochs[j + 1]))
            G2 = make_G(dir)
            flag = False
        else:
            G1 = G2
