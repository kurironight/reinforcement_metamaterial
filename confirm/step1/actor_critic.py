...
class Edgethick_Actor(ActorNetwork):
    def __init__(self, emb_size=300):
        super(Edgethick_Actor, self).__init__(num_state=emb_size, num_action=2,
                                              hidden1_size=400, hidden2_size=300, init_w=3e-3)
        self.saved_actions = []

    def forward(self, h):
        h = F.relu(self.fc1(h))
        h = F.relu(self.fc2(h))
        y = 0.1+0.9*torch.sigmoid(self.fc3(h))  # var to mean todekaeru
        return y
...
def finish_episode(Critic, Edge_thickness, Critic_opt, Edge_thick_opt, gamma):
    ...
    for (action, value),  (edge_thick_mean, edge_thick_var),  R in zip(GCN_saved_actions, Edge_thickness_saved_actions, returns):
            ...
            if advantage > 0:
                ...
                if np.isin(node_num, action["which_node"]):
                    ...
                    edge_thick_mean_loss = F.l1_loss(torch.from_numpy(
                        action["edge_thickness"]).double(), torch.tensor([edge_thick_mean]).double())
                    edge_thick_var_loss = F.l1_loss(torch.from_numpy(
                        np.abs(action["edge_thickness"]-edge_thick_mean.item())).double(), torch.tensor([torch.sqrt(edge_thick_var)]).double())
