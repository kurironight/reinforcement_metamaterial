...
def save_graph_info_npy(log_dir, nodes_positions, input_nodes, input_vectors,
                        output_nodes, output_vectors, frozen_nodes,
                        edges_indices, edges_thickness):
    """グラフの情報をlog_dir内部にnpy形式で保存する保存する．
    ...
    Args:
        ...
        edges_thickness (np.array)
    """
    np.save(os.path.join(log_dir, "nodes_pos.npy"), nodes_positions)
    ...
    np.save(os.path.join(log_dir, "output_nodes.npy"), output_nodes)
    np.save(os.path.join(log_dir, "output_vectors.npy"), output_vectors)
