...
void vector_x_matrix_2(double *y, double a[][6], double *x)
{
    int i, j;
    double vxm;
    for (i = 0; i < 6; i++)
...
void get_force(double *bar_element_stress, double node[2][2], double h, double *bar_element_displacement, double E, double b)
{
    ...
    // 梁の座標系に変位を直す
    vector_x_matrix_2(bar_coord_disp, T, bar_element_displacement); //  bar_coord_disp = TU
    for (j = 0; j < 6; ++j)
        // 軸力，せんだん力，曲げモーメントの導出
        vector_x_matrix_2(bar_coord_F, K_zero, bar_coord_disp);
    // 軸応力，曲げ応力，せん断応力を求める
    bar_element_stress[0] = bar_coord_F[0] / A; //軸力
    bar_element_stress[3] = bar_coord_F[3] / A;
    bar_element_stress[1] = bar_coord_F[1] / A; //せん断応力
    bar_element_stress[4] = bar_coord_F[4] / A;
    bar_element_stress[2] = bar_coord_F[2] / ((Iz_A * A) / (h / 2)); //曲げ応力
...
void bar_fem(double **nodes_pos, int **edges_indices, double **edges_thickness, int node_num, int edge_num, int input_node_num, int *input_nodes, double **input_vectors, int frozen_node_num, int *frozen_nodes, double **displacement, double **stresses, int tmax, double eps, double E, double b)
{
    ...
    */
    ...
    double bar_element_displacement[6];
    double bar_element_stress[6];
    for (i = 0; i < edge_num; ++i)
    {
        node1 = edges_indices[i][0];
        node2 = edges_indices[i][1];
        ...
        node[1][0] = nodes_pos[node2][0];
        node[1][1] = nodes_pos[node2][1];
        bar_element_displacement[0] = x[node1 * free_D + 0];
        bar_element_displacement[1] = x[node1 * free_D + 1];
        bar_element_displacement[2] = x[node1 * free_D + 2];
...
void bar_fem_force(double **nodes_pos, int **edges_indices, double **edges_thickness, int node_num, int edge_num, int input_node_num, int *input_nodes, double **input_forces, int frozen_node_num, int *frozen_nodes, double **displacement, double **stresses, int tmax, double eps, double E, double b)
{
    ...
    //ひずみ角の条件を加えない場合
    int condition_element_num = frozen_node_num * free_D;
