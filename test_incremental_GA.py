...
from GA.GA_class import ConstraintIncrementalNodeIncrease_GA
from GA.algorithm import Customized_NSGAII
import time
import numpy as np
...
max_free_node_num = 45  # 3の倍数で
free_nodes_num = np.array([4, 5, 6, 6, 7, 8, 8, 9, 10, 10, 11, 12, 12, 13, 14, 14, 15, 16])
free_nodes_num = np.concatenate((free_nodes_num, np.arange(16, max_free_node_num + 1)))
fix_nodes_num = np.array([np.tile([i], 3) for i in range(2, 8)]).reshape((-1))
fix_nodes_num = np.concatenate((fix_nodes_num, np.ones(free_nodes_num.shape[0] - fix_nodes_num.shape[0], np.int) * 8))
    ...
    for index, (free_node_num, fix_node_num) in enumerate(zip(free_nodes_num, fix_nodes_num)):
        ...
        with ProcessPoolEvaluator(8) as evaluator:
            if index == 0:  # 一つ目のGAでは遺伝子を引き継がない
                problem = ConstraintIncrementalNodeIncrease_GA(free_node_num, fix_node_num)
                ...
                problem = ConstraintIncrementalNodeIncrease_GA(free_node_num, fix_node_num)
                prior_problem = ConstraintIncrementalNodeIncrease_GA(free_nodes_num[index - 1], fix_nodes_num[index - 1])
                algorithm = Customized_NSGAII(problem, prior_problem, gene_path=load_gene_path, population_size=parent, variator=CompoundOperator(SBX(), HUX(), PM(), BitFlip()), evaluator=evaluator)
